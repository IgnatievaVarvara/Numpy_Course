...
# 2. Привести различные способы созания объектов типа DataFrame

## Index - способ организации ссылки на данные объектов Series и DataFrame.
# Index неизменяем, упрядочен, является мультимножеством (могут быть повторяющиеся значения)
...
print(x3.dropna()) # drop NA (Not Available)
...
print(df.dropna())
print('')
print(df.dropna(axis=0)) # по строкам
print('')
