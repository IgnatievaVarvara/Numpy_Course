...
# Набор правил, позволяющих осуществлять бинарные операции с массивами разных форм и размеров

a = np.array([0,1,2])
b = np.array([5,5,5])

print(a + b)
print(a + 5) # '5' транслируется в массив [5,5,5] => она подстраивается под размер массива a
...
# 2. Если формы массивов не совпадают в каком-либо измерении, то если форма у массива = 1,
# то он "расстягивается" до соответствия формы второго массива
# 3. Если в каком-либо измерении размеры отличаются и ни один из них != 1, то генерируется ошибка
a = np.array([[0,1,2], [3,4,5]])
b = np.array([5])

print(a.ndim, a.shape) # измерения, размеры
...
# Q2. Пример для y. Вычислить количество элементов (по обеим размерностям), значения которых > 3 и < 9
